...
<Task>
Help them categorize the problem type. Ask guiding questions. Do NOT give away the solution.
</Task>
...
<Task>
Suggest data structures/algorithms. Explain WHY they fit. Do NOT give implementation details.
</Task>
...
<Task>
Describe the solving approach. Explain intuition. Do NOT give code.
</Task>
...
<Task>
Point out what's WRONG or MISSING in structure. Do NOT write the fix.

**IMPORTANT: If failed test cases are shown, prioritize analyzing those actual failures before discussing hypothetical issues.**
</Task>
...
<Task>
Point to SPECIFIC lines/issues. Explain what's wrong. Do NOT give the complete solution.

**CRITICAL: If failed test cases are shown above:**
1. FIRST analyze the ACTUAL test failures (Input/Expected/Got)
2. Look for simple type mismatches (int vs float, string formatting, etc.)
3. Focus on what's ACTUALLY failing, not hypothetical edge cases
4. Only discuss potential issues if ALL tests pass
</Task>
...
class FeedbackAgent:
    ...
    async def generate_hint(
        ...
        Args:
            ...
            last_run_state: Optional dict with last run info (status, passed_count, etc.) - excludes code
        ...
        logger.info(f"🔍 DEBUG - last_run_state received: {last_run_state}")
        
        # Get appropriate prompt template
        template_str = self.hint_templates[hint_level]
        prompt = ChatPromptTemplate.from_template(template_str)
        
        # Prepare context with last run state
        error_text = error_message or ""
        
        # Add last run context if available (excludes code which we already have)
        if last_run_state:
            logger.info(f"🔍 Processing last_run_state: {last_run_state}")
            from app.utils.user_profiling import format_last_run_context
            last_run_context = format_last_run_context(last_run_state)
            logger.info(f"🔍 Formatted last_run_context length: {len(last_run_context)}")
            logger.info(f"🔍 Formatted last_run_context content: {last_run_context[:200]}")
            if last_run_context:
                error_text = f"{error_text}{last_run_context}"
                logger.info(f"📊 Added last run context: {len(last_run_context)} chars")
            else:
                logger.warning("⚠️ format_last_run_context returned empty string")
        else:
            logger.warning("⚠️ last_run_state is None or falsy")
        ...
        print(f"\n{'='*80}")
        print(f"📤 COMPLETE PROMPT BEING SENT TO GEMINI (Level {hint_level})")
        print(f"{'='*80}")
        prompt_str = str(formatted_prompt[0].content) if formatted_prompt else "No prompt"
        print(prompt_str)
        print(f"{'='*80}")
        print(f"Total prompt length: {len(prompt_str)} characters")
        print(f"{'='*80}\n")
